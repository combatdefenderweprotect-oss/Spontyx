...
> "Continue Spontix development. Read `CLAUDE.md` for the rules and architecture pointers. The full history is in [docs/CHANGELOG_RECENT.md](docs/CHANGELOG_RECENT.md); domain specs are in the docs map. All migrations 001–054 applied; all Edge Functions deployed (`ensure-prematch` + `generate-questions`). Pre-match generation is event-based (league create + league.html open) with cron as backstop. Per-match PREMATCH question count is user-controlled via `prematch_questions_per_match` (1–10, default 5); pipeline fills shortfall with AI then fallback templates. Per-match LIVE question count is user-controlled via `live_questions_per_match` (1–10, default 6, migration 054); generation is slot-paced via `computePlannedSlots()` (floor(N/2) pre-HT in [10–40], ceil(N/2) post-HT in [55–85]). Prematch quality filter is two-pass (text/category pre-filter + post-predicate strict filter with market dedup, lineup-aware player rules, heavy-favourite reject, 11 fallback markets). Post-predicate filter uses per-match `prematchBatchCtx` + `lineupCtx` (multi-match batch correctness). Next priorities: (1) Stripe subscriptions; (2) Arena push notification deep-links; (3) League Completion Evaluator."
