...
> "Continue Spontix development. Read `CLAUDE.md` for the rules and architecture pointers. The full history is in [docs/CHANGELOG_RECENT.md](docs/CHANGELOG_RECENT.md); domain specs are in the docs map. All migrations 001–054 applied; all Edge Functions deployed (`ensure-prematch` + `generate-questions` v68, `resolve-questions` updated 2026-05-05, `sync-fixtures` updated 2026-05-05). Pre-match generation is event-based (league create + league.html open) with cron as backstop. Per-match PREMATCH question count is user-controlled via `prematch_questions_per_match` (1–10, default 5); pipeline fills shortfall with AI then fallback templates. Per-match LIVE question count is user-controlled via `live_questions_per_match` (1–10, default 6, migration 054); generation is slot-paced via `computePlannedSlots()` (floor(N/2) pre-HT in [10–40], ceil(N/2) post-HT in [55–85]). LIVE pipeline has two post-predicate guards before insert: (1) window overlap guard (`live_window_overlap`) — rejects overlapping `match_stat_window` ranges; (2) live quality filter (`lib/live-quality-filter.ts`) — 5 hard-reject rules (already-resolved clean sheet/BTTS, blowout winner, equaliser in blowout, consecutive same market for time-driven) + soft scoring for event-driven. `matchQuestions` in `LiveMatchContext` feeds both the filter and `recentCategories` in the context packet. Prematch quality filter is two-pass (text/category pre-filter + post-predicate strict filter with market dedup, lineup-aware player rules, heavy-favourite reject, 11 fallback markets). Post-predicate filter uses per-match `prematchBatchCtx` + `lineupCtx` (multi-match batch correctness). `league.html` has two frontend moment layers: (a) Moment Feeling Layer — flash strip + slide-in entrance + live context banner on new LIVE question arrival; (b) Result Moment Layer — tension strip for closed LIVE questions, correct/wrong in-card overlay with +pts and streak copy, smooth holding card fade-in after resolve (commit f05e61b). REAL_WORLD pipeline: two critical bugs fixed 2026-05-05 — (1) `match_lineup` `resolvesAfter` was `kickoff` instead of `kickoff + 91min` (silent Stage 3 failure for all TYPE 1 questions); (2) `manual_review` skipped at generation + resolver until admin review UI ships (always auto-voids — prevents quota burn and ~49 hourly OpenAI calls per question). 0 REAL_WORLD questions existed before fix. First end-to-end verification window: 2026-05-07 ~12:00 UTC (Liverpool vs Chelsea). Sprint 1 production cleanup deployed 2026-05-05 (commit 8712565): all MVP_ constant names removed (`UNSUPPORTED_SPORTS`, `MAX_ACTIVE_LIVE_QUESTIONS`, skip reason `sport_not_supported`); `sync-fixtures` uses dynamic `getCurrentSeason()` instead of hardcoded 2025; dashboard plan panel wired to real Supabase data (leagues count, questions this week, real-world / month). Next priorities: (1) Stripe subscriptions; (2) Arena push notification deep-links; (3) League Completion Evaluator; (4) Sprint 2 from roadmap (sync_leagues config table, BR mode column, dashboard usage bars full wire-up)."
