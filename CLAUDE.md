...
> "Continue Spontix development. Read `CLAUDE.md` for the rules and architecture pointers. The full history is in [docs/CHANGELOG.md](docs/CHANGELOG.md); domain specs are in the docs map. All migrations 001–053 applied; all Edge Functions deployed (`ensure-prematch` + latest `generate-questions`). Pre-match generation is event-based (league create + league.html open) with cron as backstop. Per-match question count is user-controlled via `prematch_questions_per_match` (1–10, default 5); pipeline fills shortfall with AI then fallback templates. Next priorities: (1) Stripe subscriptions; (2) Arena push notification deep-links; (3) League Completion Evaluator."
